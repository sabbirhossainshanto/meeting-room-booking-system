...
export const initiatePayment = async (paymentData: TPayment) => {
  const res = await axios.post(config.payment_url as string, {
    ...
    success_url: `https://meeting-room-booking-system-navy.vercel.app/api/success?transactionId=${paymentData.transactionId}&status=success`,
    fail_url: `https://meeting-room-booking-system-navy.vercel.app/api/success?status=failed`,
